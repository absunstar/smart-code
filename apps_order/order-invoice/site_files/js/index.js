app.controller("order_invoice", function ($scope, $http, $timeout) {
      ...
      if (shift) {
        ...
        if ($scope.defaultSettings.general_Settings) {
          if ($scope.defaultSettings.general_Settings.order_type) {
            $scope.order_invoice.transaction_type =
              $scope.defaultSettings.general_Settings.order_type;
            ...
            if ($scope.defaultSettings.general_Settings.order_type.id == 2) {
              if ($scope.defaultSettings.general_Settings.delivery_employee) {
                $scope.order_invoice.delivery_employee =
                  $scope.deliveryEmployeesList.find((_deliveryEmployees) => {
                    return (
                      _deliveryEmployees.id ===
                      $scope.defaultSettings.general_Settings.delivery_employee
                        .id
                    );
                  });
              }
            } else if (
              $scope.defaultSettings.general_Settings.order_type.id == 1
            ) {
              if ($scope.defaultSettings.general_Settings.service) {
                $scope.order_invoice.service =
                  $scope.defaultSettings.general_Settings.service;
              }
            }
          }

          if (
            $scope.defaultSettings.general_Settings.customer &&
            $scope.defaultSettings.general_Settings.customer.id
          ) {
            $scope.getCustomersGet(
              $scope.defaultSettings.general_Settings.customer
            );
            if ($scope.defaultSettings.general_Settings.customer.gov)
              $scope.order_invoice.gov = $scope.govList.find((_gov) => {
                return (
                  _gov.id ===
                  $scope.defaultSettings.general_Settings.customer.gov.id
                );
              });

            if ($scope.defaultSettings.general_Settings.customer.city)
              $scope.getCityList(
                $scope.defaultSettings.general_Settings.customer.gov
              );

            if ($scope.defaultSettings.general_Settings.customer.area) {
              $scope.getAreaList(
                $scope.defaultSettings.general_Settings.customer.city
              );
            }

            if ($scope.defaultSettings.general_Settings.customer.address)
              $scope.order_invoice.address =
                $scope.defaultSettings.general_Settings.customer.address;

            if ($scope.defaultSettings.general_Settings.customer.phone)
              $scope.order_invoice.customer_phone =
                $scope.defaultSettings.general_Settings.customer.phone;

            if ($scope.defaultSettings.general_Settings.customer.mobile)
              $scope.order_invoice.customer_mobile =
                $scope.defaultSettings.general_Settings.customer.mobile;
          }
        }
      } else {
  ...
  $scope.addOrderInvoice = function (order_invoice, type) {
    ...
    if (order_invoice.shift) {
      if (
        (order_invoice.book_list && order_invoice.book_list.length > 0) ||
        type === "table"
      ) {
        ...
        if (
          !order_invoice.transaction_type ||
          (order_invoice.transaction_type && !order_invoice.transaction_type.id)
        ) {
        ...
        if (
          order_invoice.transaction_type.id != 2 &&
        ...
        if (
          order_invoice.transaction_type.id != 2 &&
          order_invoice.price_delivery_service
        )
          ...
          $http({
            ...
            function (response) {
              ...
              if (
                ...
                $scope.order_invoice.status.id == 2 &&
                !$scope.order_invoice.invoice
              ) {
        ...
        _order_invoice.book_list.forEach((item_book) => {
          if (item_book.kitchen && !item_book.printed) {
            if (item_book.kitchen.id == _kitchen.id) {
              let extras = "";
              if (
                item_book.extras_items_list &&
                item_book.extras_items_list.length > 0
              ) {
    ...
    $http({
      ...
      function (response) {
        ...
        if (response.data.done) {
          ...
          if (
            $scope.account_invoices.source_type.id == 3 &&
  ...
  $scope.printAccountInvoive = function (obj) {
    ...
    if ($scope.defaultSettings.printer_program) {
      ...
      if (
        $scope.defaultSettings.printer_program.thermal_header &&
        $scope.defaultSettings.printer_program.thermal_header.length > 0
      ) {
    ...
    if (obj.total_value_added) {
      obj_print.data.push({
        type: "text2",
        value2: `${site.addSubZero(obj.total_value_added, 2)}  (${
          $scope.defaultSettings.inventory.value_added || 0
        }%)`,
    ...
    if (obj.net_value) {
      obj_print.data.push(
        ...
        },
        {
          type: "invoice-total",
          value:
            site.stringfiy(obj.net_value) +
            ($scope.defaultSettings.accounting.end_num_to_str || 0),
        },
        {
          type: "line",
        }
    ...
    if (
      $scope.defaultSettings.printer_program &&
      $scope.defaultSettings.printer_program.thermal_footer &&
      $scope.defaultSettings.printer_program.thermal_footer.length > 0
    ) {
      ...
      if (shift) {
        $scope.getOrderInvoicesActiveList(() => {
          ...
          $scope.invoicesActivelist.forEach((_invoiceActive) => {
            if (
              _invoiceActive.transaction_type &&
          ...
          if (
            $scope.invoicesActivelist &&
            $scope.invoicesActivelist.length < 1
          ) {
  ...
  $scope.closeOrder = function () {
    ...
    $scope.getStockItems(
      ...
      (callback) => {
        if (!callback) {
          $scope.order_invoice.shift =
            $scope.order_invoice.shift || $scope.shift;
  ...
  $scope.bookList = function (item) {
    ...
    if (item.branches_list && item.branches_list.length > 0) {
          ...
          if (_branch.kitchen && _branch.kitchen.id) {
            ...
            if ($scope.defaultSettings.general_Settings.kitchen)
              kitchenBranch = $scope.kitchensList.find((_kitchen) => {
                return (
                  _kitchen.id ===
                  $scope.defaultSettings.general_Settings.kitchen.id
                );
              });
          }
      ...
      if ($scope.defaultSettings.general_Settings.kitchen)
        kitchenBranch = $scope.kitchensList.find((_kitchen) => {
          return (
            _kitchen.id === $scope.defaultSettings.general_Settings.kitchen.id
          );
        });
    }
    ...
    let indxUnit = item.size_units_list.findIndex(
      (_unit) => _unit.id == item.main_unit.id
    );
    item.value_added = item.not_value_added
      ? 0
      : $scope.defaultSettings.inventory.value_added || 0;
    ...
    if (!exist && !foundHold) {
      ...
      /*   if (obj.discount.type == "number") {
        obj.discount.current = obj.discount.value;
      } else if (obj.discount.type == "percent") {
        obj.discount.current = (obj.discount.value * obj.price) / 100;
      }

      obj.b_price = obj.price - obj.discount.current;
      obj.b_price = site.toNumber(obj.b_price);

      obj.total = obj.b_price * 1 + obj.extras_price * 1 + obj.total_v_a; */

      $scope.order_invoice.book_list.push(obj);
      $scope.calcSize(
        $scope.order_invoice.book_list[
          $scope.order_invoice.book_list.length - 1
        ]
      );
    }
  };
  ...
  $scope.deleteItemsList = function (item) {
    ...
    $scope.order_invoice.book_list.splice(
      $scope.order_invoice.book_list.indexOf(item),
      1
    );
    /*   if (item.count == 1) {
  ...
  $scope.calcSize = function (_size) {
    ...
    $timeout(() => {
      ...
      _size.total_v_a =
        (_size.value_added * (_size.b_price * _size.count)) / 100;
      _size.total_v_a = site.toNumber(_size.total_v_a);

      _size.total =
        _size.b_price * _size.count +
        _size.extras_price * _size.count +
        _size.total_v_a;
    ...
    $timeout(() => {
      ...
      if (obj.discountes && obj.discountes.length > 0) {
        obj.discountes.forEach((ds) => {
          if (ds.type === "percent")
            obj.total_discount +=
              (site.toNumber(obj.total_value) * site.toNumber(ds.value)) / 100;
      ...
      if (obj.transaction_type && obj.transaction_type.id == 2) {
        obj.service = 0;
        obj.price_delivery_service =
          site.toNumber(obj.price_delivery_service) || 0;
      }
      ...
      console.log(obj.total_value, obj.net_value, "ddddddddddddddddddd");
      ...
      $http({
        ...
        function (response) {
          ...
          $http({
            ...
            function (response) {
              $scope.order_invoice.count_person =
                $scope.order_invoice.count_person || 1;
            },
            function (err) {
              $scope.busy = false;
              $scope.error = err;
  ...
  $scope.changeTransactionType = function (order) {
    ...
    if (order.transaction_type) {
      if (order.transaction_type.id === 1) {
        ...
        if ($scope.defaultSettings.general_Settings.delivery_employee) {
          order.delivery_employee = $scope.deliveryEmployeesList.find(
            (_deliveryEmployees) => {
              return (
                _deliveryEmployees.id ===
                $scope.defaultSettings.general_Settings.delivery_employee.id
              );
            }
          );
        }
