      ...
      if (shiftCb) {
        ...
        stores_item_doc.sizes.forEach((_size) => {
          ...
          _size.opening_palnce_list.forEach((_opBalance) => {
            ...
            let unit = _size.size_units_list.find((_unit) => {
              return _unit.id === _opBalance.unit.id;
            });

            let discount = 0;

            if (_opBalance.count) {
              if (unit.discount.type == 'number') discount = (unit.discount.value || 0) * _opBalance.count;
              else if (unit.discount.type == 'percent') discount = ((unit.discount.value || 0) * (unit.cost * _opBalance.count)) / 100;
            }
                              ...
                              result.doc.items.forEach((_itm, i) => {
                                ...
                                if (result.doc.posting) {
                                  ...
                                  if (result.doc.type.id == 4) {
                                    ...
                                    if (result.doc.type.id == 1) _itm.set_average = 'sum_average';
                                    _itm.type = 'sum';
                                    _itm.transaction_type = 'in';
                                    site.quee('item_transaction + items', { ..._itm });
                                  }
                                  ...
                                  if (result.doc.type.id == 4) {
                                    ...
                                    if (result.doc.type.id == 1) _itm.set_average = 'minus_average';
                                    _itm.type = 'minus';
                                    _itm.count = -Math.abs(_itm.count);
                                    _itm.transaction_type = 'in';
                                    site.quee('item_transaction + items', { ..._itm });
          ...
          doc.return_paid.items.forEach((_itemsDoc) => {
            if (_itemsObj.barcode === _itemsDoc.barcode && _itemsObj.size_ar == _itemsDoc.size_ar) {
              ...
              if (_itemsDoc.discount) {
                if (_itemsDoc.discount.type == 'number') discount = _itemsDoc.discount.value * _itemsDoc.count;
