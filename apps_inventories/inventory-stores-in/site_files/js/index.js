app.controller('stores_in', function ($scope, $http, $timeout) {
    ...
    $scope.print = function () {
        ...
        if ($scope.defaultSettings.printer_program.a4_printer) {
            ...
            if ($scope.store_in.items.length > 6) {
                $scope.invList = [];
                let inv_length = $scope.store_in.items.length / 6;
                inv_length = parseInt(inv_length);
                let ramain_items = $scope.store_in.items.length - inv_length * 6;
            ...
            $scope.localPrint = function () {
                ...
                if ($scope.defaultSettings.printer_program.place_qr) {
                    if ($scope.defaultSettings.printer_program.place_qr.id == 1) {
                        site.qrcode({
                            selector: document.querySelectorAll('.qrcode:last-child')[$scope.invList.length - 1],
                            text: document.location.protocol + '//' + document.location.hostname + `/qr_storeout?id=${$scope.store_in.id}`,
                        });
                    } else if ($scope.defaultSettings.printer_program.place_qr.id == 2) {
