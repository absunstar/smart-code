    ...
    $http({
      ...
      function (response) {
        ...
        if (response.data.done) {
          if (
            account_invoices.source_type.id == 1 &&
...
/*   $scope.print = function () {
    ...
    if ($scope.defaultSettings.printer_program.a4_printer) {
      ...
      $scope.localPrint = function () {
        if (document.querySelectorAll('.qrcode').length !== $scope.invList.length) {
          ...
          if ($scope.defaultSettings.printer_program.place_qr) {
            if ($scope.defaultSettings.printer_program.place_qr.id == 1) {
              site.qrcode({ selector: document.querySelectorAll('.qrcode:last-child')[$scope.invList.length - 1], text: document.location.protocol + '//' + document.location.hostname + `/qr_storeout?id=${$scope.store_in.id}` });
            } else if ($scope.defaultSettings.printer_program.place_qr.id == 2) {
  ...
  $scope.patchesList = function (itm) {
    ...
    if ($scope.store_in.type && $scope.store_in.type.id == 4) {
      ...
      if (!$scope.item_patch.patch_list) {
        if ($scope.item_patch.work_serial) {
          ...
          for (let i = 0; i < $scope.item_patch.count; i++) {
            ...
            $scope.item_patch.patch_list.push({
              patch:
                mini_code + r_code2 + ($scope.item_patch.patch_list.length + i),
              count: 1,
            });
          }
        } else {
          $scope.item_patch.patch_list = [
            {
              ...
              expiry_date: new Date(
                $scope.addDays(new Date(), itm.validit || 0)
              ),
              count: itm.count,
        ...
        $scope.item_patch.patch_list &&
        $scope.item_patch.patch_list.length == 1 &&
        $scope.item_patch.work_patch
      ) {
        $scope.item_patch.patch_list[0].count = itm.count;
      } else {
        if ($scope.item_patch.work_serial) {
          let count =
            $scope.item_patch.count - $scope.item_patch.patch_list.length;
          let r_code2 = Math.floor(Math.random() * 1000 + 1);
    ...
    $timeout(() => {
      ...
      if (str == "exp") {
        let diffTime = Math.abs(
          new Date(i.expiry_date) - new Date(i.production_date)
        );
        i.validit = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
      } else if (str == "pro") {
        i.expiry_date = new Date(
          $scope.addDays(i.production_date, i.validit || 0)
        );
      }
    }, 250);
  };
      ...
      if (shift) {
        ...
        if ($scope.defaultSettings.general_Settings) {
          if ($scope.defaultSettings.general_Settings.vendor)
            $scope.store_in.vendor =
              $scope.defaultSettings.general_Settings.vendor;
        ...
        if ($scope.defaultSettings.inventory) {
          ...
          if ($scope.defaultSettings.accounting) {
            if (
              ...
              if ($scope.defaultSettings.accounting.payment_method) {
                $scope.store_in.payment_method =
                  $scope.defaultSettings.accounting.payment_method;
                $scope.loadSafes(
                  $scope.store_in.payment_method,
                  $scope.store_in.currency
                );
                if (
                  $scope.store_in.payment_method &&
                  $scope.store_in.payment_method.id == 1
                )
                  $scope.store_in.safe =
                    $scope.defaultSettings.accounting.safe_box;
                else
  ...
  $scope.add = function () {
    ...
    if ($scope.store_in.payment_type) {
      ...
      if (
        $scope.store_in.paid_up < $scope.store_in.amount_currency &&
        $scope.store_in.payment_type.id == 1
      ) {
        $scope.error = "##word.amount_must_paid_full##";
        return;
      }
          ...
          if (!is_allowed_date) {
            ...
            $http({
              ...
              function (response) {
                ...
                if (response.data.done) {
                  if (
                    ($scope.store_in.type.id == 1 ||
                      $scope.store_in.type.id == 4) &&
                    $scope.defaultSettings.accounting &&
                    $scope.defaultSettings.accounting.create_invoice_auto &&
                    !$scope.defaultSettings.general_Settings.work_posting
                  ) {
    ...
    if ($scope.store_in.type) {
      ...
      if ($scope.item.sizes && $scope.item.sizes.length > 0)
        $scope.item.sizes.forEach((_size) => {
          foundSize = $scope.store_in.items.some(
            (_itemSize) =>
          ...
          if (_size.count > 0 && !foundSize) {
            ...
            _size.total_v_a =
              (_size.value_added * (_size.cost * _size.count)) / 100;
            ...
            if (_size.count) {
              if (_size.discount.type == "number")
                ...
                discount =
                  ((_size.discount.value || 0) * (_size.cost * _size.count)) /
                  100;
      ...
      if (calc_size.count) {
        if (calc_size.discount.type == "number") {
          ...
          calc_size.discount.current =
            (calc_size.discount.value * calc_size.cost) / 100;
        }
        ...
        calc_size.total_v_a =
          (calc_size.value_added * (calc_size.b_cost * calc_size.count)) / 100;
        calc_size.total_v_a = site.toNumber(calc_size.total_v_a);
        calc_size.total =
          calc_size.b_cost * calc_size.count + calc_size.total_v_a;
                  ...
                  _item.sizes.forEach((_size) => {
                    ...
                    if (
                          ...
                          if (
                            ...
                            if (
                              ...
                              _size.branches_list[indxBranch].stores_list.map(
                                (_store, i) => {
                                  if (
                                    $scope.store_in.store &&
                                    _store.store.id == $scope.store_in.store.id
                                  ) {
                                    foundStore = true;
                                    indxStore = i;
                      ...
                      if (!foundSize && !foundHold)
                        $scope.item.sizes.unshift(_size);
                    }
                  });
              });
  ...
  $scope.delete = function (store_in) {
    ...
    if (
      store_in.return_paid &&
      store_in.net_value != store_in.return_paid.net_value
    ) {
              ...
              function (response) {
                ...
                if (response.data.done) {
                  ...
                  if (response.data.error.like("*OverDraft Not*")) {
                    ...
                    response.data.error.like("*t`s Have Account Invo*")
                  ) {
      ...
      for (let i = 0; i < _store_in_all.length; i++) {
        setTimeout(() => {
          if (!_store_in_all[i].posting) {
            $scope.getStockItems(
              _store_in_all[i].items,
              _store_in_all[i].store,
              (callback) => {
  ...
  */
  ...
  $scope.getNumberingAuto = function () {
    ...
    if (site.toNumber("##query.type##")) {
      if (site.toNumber("##query.type##") == 1)
        screen = "purchases_invoices_store";
      else if (site.toNumber("##query.type##") == 2) screen = "depts_store";
      else if (site.toNumber("##query.type##") == 3)
        screen = "opening_balances_Store";
      else if (site.toNumber("##query.type##") == 4)
        screen = "return_purchases_store";
