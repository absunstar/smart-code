app.controller("stores_items", function ($scope, $http, $timeout) {
  ...
  $scope.addSize = function () {
    ...
    if (
      ...
      !$scope.defaultSettings.inventory.auto_barcode_generation
    ) {
      ...
      let err_barcode1 = $scope.itemSizeList.some(
        (_itemSize) => _itemSize.barcode === $scope.item.barcode
      );
      let err_barcode2 = $scope.category_item.sizes.some(
        (_itemSize) => _itemSize.barcode === $scope.item.barcode
      );
    ...
    if (
      $scope.item.discount &&
      $scope.item.discount.value > $scope.item.discount.max
    ) {
    ...
    $http({
      ...
      function (response) {
        ...
        if (response.data.done) {
          ...
          $scope.category_item.sizes.splice(
            $scope.category_item.sizes.indexOf(itm),
            1
          );
      },
      function (err) {
            ...
            $scope.list.forEach((_item) => {
              _item.sizes.forEach((_sizes) => {
                if (
                  _sizes &&
                  ((_sizes.size_ar &&
                    _sizes.size_ar.contains($scope.hideObj.size_ar)) ||
                    (_sizes.size_en &&
                      _sizes.size_en.contains($scope.hideObj.size_en)) ||
                    (_sizes.barcode &&
                      _sizes.barcode === $scope.hideObj.barcode))
                ) {
                  _sizes.$hide = false;
                } else {
                  _sizes.$hide = true;
  ...
  $scope.newCategoryItem = function () {
    ...
    if ($scope.defaultSettings.inventory) {
      ...
      if ($scope.defaultSettings.inventory.item_type)
        $scope.category_item.item_type =
          $scope.defaultSettings.inventory.item_type;
  ...
  $scope.add = function () {
    ...
    if (
      ($scope.category_item.sizes && $scope.category_item.sizes.length > 0) ||
      !$scope.category_item.add_sizes
    ) {
      $scope.busy = true;
      $http({
        method: "POST",
  ...
  $scope.addOpeningBalance = function (size, type) {
    ...
    if (type === "add") {
      if (
        $scope.size_balance.opening_palnce_list &&
        $scope.size_balance.opening_palnce_list.length > 0
      ) {
  ...
  $scope.incertComplexItem = function (item) {
    ...
    foundSize = item.complex_items.some(
      (_itemSize) => _itemSize.barcode === $scope.items_size.barcode
    );
  ...
  $scope.complexItemsPushUpdate = function () {
    $scope.error = "";
    if (
      $scope.complexItemsUpdate &&
      $scope.complexItemsUpdate &&
      $scope.complexItemsUpdate.length > 0
    )
  ...
  $scope.addUnitsList = function (_u) {
    ...
    let found1 = $scope.category_item.units_list.some(
      (_unit) => _unit.id == u.id
    );
  ...
  $scope.deleteUnit = function (unit) {
    ...
    if (!found)
      $scope.category_item.units_list.splice(
        $scope.category_item.units_list.indexOf(unit),
        1
      );
  };
  ...
  $scope.addMainUnit = function (category_item) {
    ...
    if (category_item.main_unit) {
      let found = category_item.units_list.some(
        (_unit) => _unit.id == category_item.main_unit.id
      );
      if (!found) {
        let objUnit = {
      ...
      category_item.sizes.forEach((_size) => {
        ...
        let found = _size.size_units_list.some(
          (_unit) => _unit.id == category_item.main_unit.id
        );
                ...
                if (
                  ...
                  response.data.list[0].sizes.forEach((_size) => {
                    ...
                    if (
                      ...
                      _size.size_units_list.forEach((_unit, i) => {
                        if (
                          _unit.barcode === c.barcode &&
                          typeof _unit.barcode == "string"
                        ) {
                          foundUnit = true;
                        }
                    ...
                    if (_size.barcode === c.barcode || foundUnit) {
                            ...
                            if (
                              ...
                              if (foundStore)
                                _size.branches_list[indxBranch].stores_list[
                                  ...
                                  if (
                                    _unit.id ==
                                    response.data.list[0].main_unit.id
                                  )
                      ...
                      foundSize = $scope.units_switch.items.some(
    ...
    if ($scope.units_switch.items.length > 0) {
      ...
      $scope.units_switch.items.forEach((_sW_i) => {
        if (
          _sW_i.unit &&
          _sW_i.units_trans &&
          _sW_i.unit.id == _sW_i.units_trans.id
        ) {
          sameUnit = true;
        }
  ...
  $scope.patchesListSwitch = function (itm) {
    ...
    if (itm.units_trans && itm.units_trans.id) {
      ...
      if (!$scope.item_patch.patch_trans_list) {
          ...
          for (let i = 0; i < $scope.item_patch.count_trans; i++) {
            ...
            $scope.item_patch.patch_trans_list.push({
              patch:
                mini_code +
                r_code2 +
                ($scope.item_patch.patch_trans_list.length + i),
              count: 1,
            });
          }
        } else {
        ...
        $scope.item_patch.patch_trans_list &&
        $scope.item_patch.patch_trans_list.length == 1 &&
        ...
        if ($scope.item_patch.work_serial) {
          let count =
            $scope.item_patch.count_trans -
            $scope.item_patch.patch_trans_list.length;
          let r_code2 = Math.floor(Math.random() * 1000 + 1);
    ...
    $scope.getSerialList(unitsSwitch.items, (serial_list) => {
      ...
      unitsSwitch.items.forEach((_item) => {
        if (_item.size_units_list && _item.size_units_list.length > 0) {
          ...
          if (
            count != _item.count_trans &&
            (_item.work_serial || _item.work_patch)
          ) {
            obj.patchCount = true;
            obj.patch_list.push(_item.barcode);
          }
  ...
  $scope.calc = function (obj) {
    $timeout(() => {
      ...
      if (obj.units_trans && obj.units_trans.id) {
        obj.count_trans =
          (obj.unit.convert * obj.count) / obj.units_trans.convert;
      } else {
        obj.count = 1;
        obj.count_trans = 0;
  ...
  $scope.calcTrance = function (obj) {
    $timeout(() => {
      ...
      if (obj.units_trans && obj.units_trans.id) {
        obj.count =
          (obj.units_trans.convert * obj.count_trans) / obj.unit.convert;
      } else {
        obj.count = 1;
        obj.count_trans = 0;
        $scope.error = "##word.err_units_trans##";
