        ...
        if (setting.siteTemplate.id == 1) {
            ...
            site.searchArticles({ search: query, tag: tag, host: options.filter, page: page, limit: limit }, (err, result) => {
                if (!err && result) {
                    ...
                    if (result.tag) {
                        options.page_title = `${language.siteName} ${language.titleSeparator} ${req.word('search results for tag ')} "${result.tag}"  ( ${result.count} articles ) [ page ${
                            result.page
                        } of ${options.pageCount} ]`;
                    } else {
                        options.page_title = `${language.siteName} ${language.titleSeparator} ${req.word('search results for ')} "${result.search}"  ( ${result.count} articles ) [ page ${
                            result.page
                        } of ${options.pageCount} ]`;
                    }
