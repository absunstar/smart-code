module.exports = function init(site) {
  ...
  site.post('/api/report_payable_vendors/all', (req, res) => {
    let response = {
      done: false,
    };
        ...
        if (!err && docs && docs.length > 0) {
          ...
          docs.forEach((_doc) => {
            ...
            _doc.payable_list.forEach((_p) => {
              if (_p.date && _p.value) {
                if (
                  (date1 && new Date(_p.date) >= new Date(date1) && new Date(_p.date) <= date1.setDate(date1.getDate() + 1)) ||
                  (date_to && date_from && new Date(_p.date) <= new Date(date_to) && new Date(_p.date) >= new Date(date_from)) ||
                  (!date1 && !date_to && !date_from)
                ){
                  if (_p.value !== _p.paid_up) {
                    list.push({
                      id: _doc.id,
