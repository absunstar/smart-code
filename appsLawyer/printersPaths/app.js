  ...
  if (app.allowRoute) {
    if (app.allowRouteGet) {
      site.get(
        ...
        (req, res) => {
          res.render(app.name + "/index.html", { title: app.name, appName: req.word("Printers Paths"), setting: site.getSiteSetting(req.host) }, { parser: "html", compres: true });
        }
      );
    }
    ...
    if (app.allowRouteAll) {
      site.post({ name: `/api/${app.name}/all`, public: true }, (req, res) => {
        ...
        if (app.allowMemory) {
          ...
          let list = app.memoryList.filter((g) => (typeof where.active != "boolean" || g.active === where.active) && host == g.host && JSON.stringify(g).contains(search)).slice(0, limit);
          res.json({
            done: true,
            list: list,
          });
