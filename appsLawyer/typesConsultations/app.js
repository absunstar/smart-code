    ...
    if (app.allowRoute) {
        if (app.allowRouteGet) {
            site.get(
                ...
                (req, res) => {
                    res.render(app.name + '/index.html', { title: app.name, appName: req.word('Types Consultations'), setting: site.getSiteSetting(req.host) }, { parser: 'html', compres: true });
                }
            );
        }
        ...
        if (app.allowRouteAll) {
            site.post({ name: `/api/${app.name}/all`, public: true }, (req, res) => {
                ...
                let select = req.body.select || { id: 1, name: 1,  price: 1, image: 1, active: 1 };
                let list = [];
                app.memoryList
                    .forEach((doc) => {
                        let obj = { ...doc };
