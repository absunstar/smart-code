module.exports = function init(site) {
  ...
  site.post("/api/operations_requests/update", (req, res) => {
    ...
    operations_requests_doc.operations_list.forEach((_operations) => {
      if (
        _operations.person_delivery &&
        _operations.person_delivery.name &&
        !_operations.delivery_date
      )
        _operations.delivery_date = new Date();
    });

    operations_requests_doc.delivery =
      operations_requests_doc.operations_list.every(
        (_a1) => _a1.person_delivery && _a1.person_delivery.name
      );
    operations_requests_doc.Putting_results =
      operations_requests_doc.operations_list.every((_a2) => _a2.result);
