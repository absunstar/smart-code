app.controller("analysis_requests", function ($scope, $http, $timeout) {
    ...
    $http({
      ...
      function (response) {
        ...
        if (response.data.done) {
          ...
          $scope.delivery_all = false;
          $scope.person_delivery = {};
        } else {
  ...
  $scope.showDeliveryAnalyzes = function (analysis_requests) {
    $scope.deliveryanalyzes = analysis_requests;
    site.showModal('#deliveryAnalyzisModal');
  ...
  $scope.getDoctorVisitAnalysis = function (doctor_visit) {
    ...
    $scope.analysisList.forEach(_an => {
      let found = doctor_visit.analysis_list.some(_a => _a.analysis.id === _an.id);
